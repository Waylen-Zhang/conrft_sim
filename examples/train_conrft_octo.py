...
def recursive_check(buffer_data, input_data, path="root"):
    ...
    # 2. 如果 input 是列表/元组，或者是 numpy 数组
    else:
        # 获取 Buffer 期望的形状
        if hasattr(buffer_data, "shape"):
            # buffer_data 通常是 (Capacity, ...)，我们关心的是后面的维度
            buffer_expected_shape = buffer_data.shape[1:] 
        else:
            # Buffer 里可能也是由 dict 组成的，如果这里不是 array，可能路径不对
            return
